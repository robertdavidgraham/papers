...
struct SMACK {
     ...
     * OPTIMIZATION: we use two-byte numbers here to remove an extra
     * assembly opcode during evaluation
     */
    unsigned short       char_to_symbol[ALPHABET_SIZE];
...
smack_add_symbol(struct SMACK *smack, unsigned c)
{
    ...
    smack->char_to_symbol[c] = (unsigned short)(symbol * sizeof(transition_t));
 ...
 ****************************************************************************/
unsigned
smack_count_patterns(const struct SMACK *smack)
{
    return smack->m_pattern_count;
}
...
smack_stage4_make_final_table(struct SMACK *smack)
{
    ...
    transition_t *table;
    unsigned short *char_to_symbol = smack->char_to_symbol;
     ...
     * NOTE: because of x86 assembly-language optimization, the minimum 
     * row-size is 8 elements
     */
    smack->y_row_shift = row_shift_from_symbol_count(smack->symbol_count);
    column_count = 1 << smack->y_row_shift;
    ...
    for (row=0; row<row_count; row++) {
        unsigned col;
        unsigned row_shift = smack->y_row_shift;

        for (col=0; col<ALPHABET_SIZE; col++) {
            transition_t transition;
            unsigned symbol = char_to_symbol[col]/sizeof(transition_t);

            transition = (transition_t)GOTO(row,col);

            *(table + row*column_count + symbol) = transition<<row_shift;
        }
    }
 ...
 ****************************************************************************/
static unsigned
handle_match(   const struct SMACK * smack,
                unsigned index,
 ...
 ****************************************************************************/
unsigned
smack_search(   const struct SMACK * smack,
                const void *v_px,
                unsigned length,
                ...
                unsigned *current_state)
{
    ...
    unsigned row;
    unsigned i;
    const unsigned short *char_to_symbol = smack->char_to_symbol;
     ...
     * variable */
    row = (*current_state & 0xFFFFFF) << smack->y_row_shift;
    ...
    for (i=0; i<length; i++) {
        ...
        column = char_to_symbol[c]/sizeof(transition_t);
    ...
    *current_state = row >> smack->y_row_shift;
    return found_count;
}
 ...
 *****************************************************************************/
size_t
smack_search_next(      const struct SMACK *  smack,
                        unsigned *      current_state,
                        const void *    v_px,
                        unsigned *       offset,
                        unsigned        length
                        )
{
    ...
    const unsigned short *char_to_symbol = smack->char_to_symbol;
    const transition_t *table = smack->y_table;
    register unsigned row_shift = smack->y_row_shift;
    const struct SmackMatches *match = smack->m_match;
    unsigned current_matches = 0;
    size_t id = (size_t)-1;
    register unsigned match_limit;

    /* Get the row. This is encoded as the lower 24-bits of the state
     * variable */
    row = (*current_state & 0xFFFFFF) << row_shift;
    assert(0 == (row>>24));
    match_limit = smack->m_match_limit << row_shift;
    ...
    if (!current_matches) {
        ...
        /* Test to see if we have one (or more) matches, and if so, call
         * the callback function */
        if (match[row >> row_shift].m_count) {
            i++; /* points to first byte after match */
            current_matches = match[row >> row_shift].m_count;
        }
    }
    ...
    *offset = i;
    ...
    if (current_matches) {
        id = match[row >> row_shift].m_ids[current_matches-1];
        current_matches--;
    }

    *current_state = (row >> row_shift) | (current_matches<<24);
    return id;
}
...
smack_benchmark(void)
{
    ...
    static uint64_t ITERATIONS = 1000;
    unsigned i;
    struct SMACK *s;
    uint64_t start, stop;
    uint64_t result = 0;
    ...
    if (result) {
        ...
        printf("clocks/byte = %5.3f-clocks\n", (1.0/cycles));
